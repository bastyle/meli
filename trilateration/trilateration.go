...
//agregar valores de inicio a archivo properties posiblemente modicable por un mantenedor
...
func (c1 Satellite) Trilaterate2(c2, c3 Satellite) (x, y float64) {
	return Trilaterate2(c1, c2, c3)
}

func Trilaterate2(c1, c2, c3 Satellite) (x, y float64) {
	d1, d2, d3, i1, i2, i3, j1, j2, j3 := &c1.d, &c2.d, &c3.d, &c1.x, &c2.x, &c3.x, &c1.y, &c2.y, &c3.y
	x = ((((math.Pow(*d1, 2)-math.Pow(*d2, 2))+(math.Pow(*i2, 2)-math.Pow(*i1, 2))+(math.Pow(*j2, 2)-math.Pow(*j1, 2)))*(2**j3-2**j2) - ((math.Pow(*d2, 2)-math.Pow(*d3, 2))+(math.Pow(*i3, 2)-math.Pow(*i2, 2))+(math.Pow(*j3, 2)-math.Pow(*j2, 2)))*(2**j2-2**j1)) / ((2**i2-2**i3)*(2**j2-2**j1) - (2**i1-2**i2)*(2**j3-2**j2)))
	y = ((math.Pow(*d1, 2) - math.Pow(*d2, 2)) + (math.Pow(*i2, 2) - math.Pow(*i1, 2)) + (math.Pow(*j2, 2) - math.Pow(*j1, 2)) + x*(2**i1-2**i2)) / (2**j2 - 2**j1)
	return x, y
...
func GetLocation2(distances ...float32) (x, y float32) {
	fmt.Println("GetLocation distances: ", distances)
	...
	xFromKenobi, yFromKenobi := kenobiSat.Trilaterate2(skywalkerSat, satoSat)
	xFromSkywalker, yFromSkywalker := skywalkerSat.Trilaterate2(satoSat, kenobiSat)
	xFromSato, yFromSato := satoSat.Trilaterate2(skywalkerSat, kenobiSat)
